...
SIDECARS=(buzz-acp buzz-agent buzz-dev-mcp buzz-marmot-sidecar git-credential-nostr buzz)
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo "Run 'cargo build --release -p buzz-acp -p buzz-agent -p buzz-dev-mcp -p buzz-marmot-sidecar -p git-credential-nostr -p buzz-cli' first." >&2
    exit 1
fi
